    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Hai @${num.split("@")[0]} 👋 Selamat Datang Di Group *${metadata.subject}* Ada *${memeg} Orang* Didalam Grup, Jangan Lupa Baca Rules Grup Yaa..` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `Selamat Tinggal @${num.split("@")[0]} 👋` })
                }
            }
        } catch (err) {
            console.log(err)
